...
class Starfield {
  ...
  _build() {
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      const mag =
        this.minMagnitude + Math.pow(random(), 0.4) * (this.maxMagnitude - this.minMagnitude);
  ...
  _randomStarTemp(r) {
    ...
    if (r < 0.5) return 3700 + r * 3000; // K: orange
...
class Moon {
  ...
  setPhaseAndPosition(phase, elevation, azimuth) {
    ...
    this.material.uniforms.sunDir.value
      .set(Math.sin(phaseAngle), 0.1, -Math.cos(phaseAngle))
      .normalize();
  }
...
function createComet(scene, options = {}) {
  ...
  const dustDir = ionDir
    .clone()
    .add(new THREE.Vector3(0.3, 0.1, 0))
    .normalize();
  const dustPoints = [];
  for (let i = 0; i < 40; i++) {
    const t = i / 39;
...
class NightSkyController {
  ...
  setNightProgress(t) {
    this.nightProgress = Math.max(0, Math.min(1, t));
  }
  setMoonPhase(phase) {
