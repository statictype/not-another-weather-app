...
- `GET /api/weather/forecast` → today + future days + astro + hourly (1 h edge TTL). Asks upstream for `days=4`; free keys cap the reply at 3 days total, so `forecast` carries 2 or 3 future days and `ForecastCard` renders whatever arrives.
...
`src/worker/cache.ts` includes `CACHE_VERSION` (currently `"7"`) in every edge cache key. If you change a shape in `src/lib/schemas.ts` without bumping it, the previously-cached entries — still valid against the _old_ upstream schema — will continue to serve from `caches.default` and render against new client expectations as `undefined` fields. There's no automated guard; bumping is a discipline thing whenever the DTO surface changes.
