...
- **Every upstream lookup resolves through one step.** `src/worker/fold.ts` folds a multi-token query to ASCII before it goes upstream — WeatherAPI matches its index in ASCII, and `tromsø, norway` otherwise returns Norway, Iowa — and copies the query's diacritics back onto the name upstream returns, so the card still reads `Tromsø`. `src/worker/locate.ts` resolves that query through `search.json` to a location id once (24 h cache) and both weather tiers fetch `q=id:N`, so `current` and `forecast` cannot land on different cities. `lat,lon` from geolocation skips both steps. See `docs/rfcs/013-ascii-location-resolution.md`.
