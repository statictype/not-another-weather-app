...
- **`UnitSystem` is named once, in `src/lib/units.ts`.** The union `"metric" | "imperial"` keys `MeasurePair` in `schemas.ts` and the store in `src/hooks/use-unit-system.ts` (a third `createSubscription` over localStorage, defaulting from `navigator.language`'s region). Not the URL — a shared `?city=` link should read in the recipient's units.
