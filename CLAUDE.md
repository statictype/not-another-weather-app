...
- **`UnitSystem` is named once, in `src/lib/units.ts`.** The union `"metric" | "imperial"` keys `MeasurePair` in `schemas.ts`, the `CLOCK` table in `src/lib/clock.ts`, and the store in `src/hooks/use-unit-system.ts` (a third `createSubscription` over localStorage, defaulting from `navigator.language`'s region). Not the URL — a shared `?city=` link should read in the recipient's units.
- **The Worker formats what comes from the payload; the client formats what comes from the clock.** Every time and date in `src/components` goes through `src/lib/clock.ts`, whose one `CLOCK` constant maps metric to `en-GB` (24-hour, `Fri 15 Aug`) and imperial to `en-US` (12-hour, `Fri Aug 15`).
